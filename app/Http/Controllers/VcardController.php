    ...
    public function getSession(Request $request)
    {
        ...
        foreach ($bookedAppointments as $appointment) {
            ...
            if ($appointment->date == $request->date) {
                if (getUserSettingValue('time_format', getLogInUserId()) == 1) {
                    ...
                    $bookedSlot[]= date('h:i A', strtotime($appointment->from_time)).' - '.date('h:i A', strtotime($appointment->to_time));
                }
            }
        }
        ...
        foreach ($WeekDaySessions as $index => $WeekDaySessions) {
            ...
            if (getUserSettingValue('time_format', getLogInUserId()) == 1) {
                ...
                $bookingSlot[] = date('h:i A', strtotime($WeekDaySessions->start_time)).' - '.date('h:i A',
                        strtotime($WeekDaySessions->end_time));
